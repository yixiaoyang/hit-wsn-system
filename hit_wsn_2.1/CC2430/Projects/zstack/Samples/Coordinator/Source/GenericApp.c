...
uint16 data_trans_process( uint8 task_id, uint16 events ) //任务处理函数
{
  ...
  char send_info[40];
  //在串口调试助手中你发什么就收到什么
  HalUARTRead(SPI_MGR_DEFAULT_PORT,buf,32 );//读串口数据到buf
  len = strlen(buf);
  HalUARTWrite(0,buf,strlen(buf));
  //uart0_puts(buf,strlen(buf));
  /*
  if(len >= 10)
  {
    //uart0_puts("len>10",6);
    if(buf[0] == '[' && buf[1] == '[' && buf[2] == 'R' && buf[3] == 'D' && buf[4] == '#')
    {
      ...
      for(i=0;i<point_num;i++)
      {
        ...
        {
          sprintf(send_info,"#%s:%s:%s:%s:%s#",point_info[i].temp,point_info[i].humidity,point_info[i].smoke,point_info[i].co,point_info[i].ch4);
          //send_info[27] = '\0';
  ...
  else if(len >= 4)
  {
    if(strcmp(buf,"[[RA")==0)
    {
      for(i=0;i<point_num;i++)
      {
        sprintf(send_info,"#%s#",point_info[i].address);
        //send_info[6] = '\0';
