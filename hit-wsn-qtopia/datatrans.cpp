		...
		if((nread = read(serial_fd,rx_buffer,RX_BUF_SIZE)) > 0)
		{
			...
			if(nread >= 6)//adress message
			{
				if( (rx_buffer[0] == '#') && (rx_buffer[5] == '#') ){
					...
					all_addr[dot_count][5] = '\0';
					
					//rand dot
					dot_pos[dot_count][0] = rand()%500;//x val
					dot_pos[dot_count][1] = rand()%250;//y val
...
int DataTrans::get_data(int node_no,int place)
{
	...
	for(int i = 0 ; i < 4; i++){
		tx_buffer[5+i] = all_addr[node_no][i];
	}
/*
	tx_buffer[5] = '1';
	tx_buffer[6] = '1';
...
*/
	...
	DEBUG_PRINT("tx_buffer=%s\r\n",tx_buffer);
	nwrite = write(serial_fd,tx_buffer,strlen(tx_buffer));
	DEBUG_PRINT("wait result\r\n");	
	...
	while(1)
	{
		...
		if((nread = read(serial_fd,rx_buffer,RX_BUF_SIZE)) > 0)
		{
			DEBUG_PRINT("len:%d,content:%s",nread,rx_buffer);
			DEBUG_PRINT("\r\n");
			...
			if(nread >= 8)//data message
			{
				...
				//get ch4 data
				memset(temp,8,0);
				for( n = 0 ; (m < nread) && (rx_buffer[m] != ':') && (rx_buffer[m] != '\0' &&(rx_buffer[m] != '#') ); m++, n++){
					temp[n] = rx_buffer[m];
				}
				temp[n+1] = '\0';
				GWsnData[place].ch4 = atof(temp);
				m++;
				DEBUG_PRINT("temp:%.2f,humidity:%.2f,smoke:%.2f,co:%.2f,ch4:%.2f\r\n",
					GWsnData[place].temp,
					GWsnData[place].humidity,
					GWsnData[place].smoke,
					GWsnData[place].co,
					GWsnData[place].ch4);
...
int DataTrans::read_uart()
{
	...
	int total = 0;
	while(1){
		if((nread = read(serial_fd,rx_buffer+total,RX_BUF_SIZE)) > 0)
		{
			DEBUG_PRINT("get str:%s\r\n",rx_buffer);
			if(		(rx_buffer[total+nread-1] == '\n') | 	
					(rx_buffer[total+nread-1] == '\0') | 
					(rx_buffer[total+nread] == '\r') )
			{
				total += nread;
				rx_buffer[total-1] = '\0';
				break;
...
int DataTrans::set_parity(int fd)
{
	...
	if(Uart1_Cfg.parity != 'n')
		options.c_iflag |= INPCK;
	options.c_cc[VTIME] = 0;	//10 seconds delay if not recieved data
	options.c_cc[VMIN] = 0;
#if 1
